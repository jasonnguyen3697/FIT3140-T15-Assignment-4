...
var excel=require('excel4node'); //Spreadsheet module using https://stackoverflow.com/questions/17450412/how-to-create-an-excel-file-with-nodejs
...
function PostObjectSize(data,string){
    objectSize=sizeof(data);
    worksheet.cell(excelcounter,1).string(string);
    worksheet.cell(excelcounter,2).number(objectSize);
    worksheet.cell(excelcounter,3).number(transactioncount);
    excelcounter+=1;
    workbook.write('Benchmark(JSON).xlsx');
}
...
io.listen(localportSocketio).on("connection", function (socket){
    ...
    socket.on('Validate',function(data){
      ...
      PostObjectSize(data,'Transaction Validation Received');
        ...
        if (validateRequest(data)){
            ...
            for (var i=1; i<servers.length;i++){
                ...
                PostObjectSize(data,'Send Addblock');
    ...
    socket.on('addBlock',function(data){
        ...
        if (isNew){
            ...
            newServerInfo.socket.send('blockchain',buf);
            PostObjectSize(buf,'Send blockChain on Join');
            var buf2 = messages.Transac.encode({
              shorten: JSON.stringify(serversCopy)
            });
            newServerInfo.socket.send('servers',buf2);
            PostObjectSize(buf2,'Send blockChain on Join');
    ...
    socket.on('servers',function(dataen){
        ...
        PostObjectSize(dataen,'Recieved Server List');
    ...
    socket.on('blockchain',function(dataen){
        ...
        PostObjectSize(dataen,'Recived BlockChain');
